...
int main()
{
    ...
    while (!glfwWindowShouldClose(window))
    {
        ...
        glStencilMask(0x00); //轮廓禁用深度写入

        outlineShader.use();
        outlineShader.set_uniform("view", 1, GL_FALSE, glm::value_ptr(view));
        outlineShader.set_uniform("projection", 1, GL_FALSE, glm::value_ptr(projection));

        //cube1 outline
        model = glm::mat4(1.0f);
        model = glm::translate(model, glm::vec3(-1.0f, 0.0f, -1.0f));
        model = glm::scale(model, glm::vec3(1.1f, 1.1f, 1.1f));
        outlineShader.set_uniform("model", 1, GL_FALSE, glm::value_ptr(model));
        glDrawArrays(GL_TRIANGLES, 0, 36);
        //cube2 outline
        model = glm::mat4(1.0f);
        model = glm::translate(model, glm::vec3(2.0f, 0.0f, 0.0f));
